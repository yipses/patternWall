...
export const contours: Generator = {
   ...
   */
  params: [
    { key: 'levels', label: 'Contour lines', type: 'number', min: 6, max: 30, step: 1, default: 8, description: 'How many heights get a line. Every one of them lands inside the terrain, because the field is stretched to the relief actually present before any height is asked of it. More lines read as steeper country, since a contour map shows slope as line density.' },
    { key: 'scale', label: 'Terrain scale', type: 'number', min: 0.6, max: 2, step: 0.1, default: 2, description: 'How far back you are standing. Low values give two or three broad massifs across the width; high values an archipelago of small islands.' },
    ...
    { key: 'weight', label: 'Line weight', type: 'number', min: 0.3, max: 1, step: 0.05, default: 0.3, description: 'Line width, scaled to the canvas so it looks the same at any export size.' },
    { key: 'indexEvery', label: 'Index contours', type: 'number', min: 0, max: 5, step: 1, default: 2, description: 'Draw every nth line heavier, the way a printed map does, so the eye can count elevation instead of only reading shape. Zero draws every line the same.' },
    { key: 'colorSpread', label: 'Colour spread', type: 'number', min: 0, max: 0.65, step: 0.01, default: 0.35, description: 'How much of the accent ramp the elevation walks through. At zero every line is the middle of the palette; at one the lowest contour and the highest sit at opposite ends of it.' },
